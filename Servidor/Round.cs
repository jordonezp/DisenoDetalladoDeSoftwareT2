...
public class Round {
  ...
  public Round(int whoDeals, int whoPlays, Deck deck, TableCards tableCards, View view, List<Player> players) {
    ...
    _pointsAssigner = new PointsAssigner(_view);
  ...
  public void VerifyIfEscobasAtDeal() {
    ...
    CardsClaimer cardsClaimer = new CardsClaimer(_tableCards,  _players[_whoDeals], point, _view, cardSubsets);
    if (!(point == Point.Zero)) {
  ...
  public void PlayTurn() {
    TurnPreparations();
    Turn turn = new Turn(_lastPlayerToHaveClaimedCards, _players[_whoPlays], _tableCards, _view);
  ...
  public void UpdateGame(Point point, List<Card> cards) {
    CardsClaimer cardsClaimer = new CardsClaimer(_tableCards, _players[_whoDeals], point, _view, new List<List<Card>>() {cards});
    UpdateCardsData(cardsClaimer);
